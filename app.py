...
def predict_nevus():
    IMAGE_SIZE = 224
    model = tf.keras.models.load_model('Training_Models/Nevus2.h5')
...
def predict_brain():
    IMAGE_SIZE = 256
    model = tf.keras.models.load_model('Training_Models/Brain_tumor.h5')
    imagefile = request.files['imagefile']
...
def predict_brain2():
    image_pth = "./static/uploaded_photo.png" 
    IMAGE_SIZE = 256
    model = tf.keras.models.load_model('D://ProjeDerin//Training_Models//Brain_tumor.h5')
    image = tf.keras.preprocessing.image.load_img(image_pth , target_size=(IMAGE_SIZE,IMAGE_SIZE))
    image = tf.keras.preprocessing.image.img_to_array(image)
    test_x = np.expand_dims(image , axis=0)
    predictions = model.predict(test_x)
